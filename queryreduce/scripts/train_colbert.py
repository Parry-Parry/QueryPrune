...
def main(args):
    ...
    for file in files:    
        ...
        out = subprocess.run([
            'python', '-m', 'colbert.train', '--amp', '--doc_maxlen', '180', '--mask-punctuation', '--maxsteps', str(len(idx)), '--triples', os.path.join(args.sink, tsv_name), '--root', args.root, '--experiment', args.experiment, '--run', strip_name
        ])
